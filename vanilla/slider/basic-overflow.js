...
window.Webflow?.push(function () {
    ...
    document.querySelector('.basic-slider').addEventListener('wheel', function(e) {
        ...
        if (!isRunning) {
            ...
            const scrollHeight = elementB.scrollHeight;
            const scrollTop = elementB.scrollTop;
            const clientHeight = elementB.clientHeight;
            if (e.deltaY > 0 && scrollTop + clientHeight >= scrollHeight - 1) {
              runAnimation(direction);
              document.querySelector('.intro-right-text').scrollTop = 0;
              $('.intro-right-text').css('overflow', 'hidden');
              setTimeout(() => {
                $('.intro-right-text').css('overflow', 'auto');
              }, 1000)
            } else if (e.deltaY < 0 && scrollTop === 0) { // Check for scrolling up and reaching top
                runAnimation(direction);
